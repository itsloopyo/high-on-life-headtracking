...
ue::FVector PositionOffset(const ue::FQuat4d& cleanRotation,
    ...
    //
    // Sway is negated for the same reason its ancestor negates it, and the
    // reasoning needs no knowledge of what the tracker calls positive: turning
    // the head rotates the face about the neck, so the tracked point slides
    // sideways by sin(yaw)*pivot, and that slide has to go the same way as the
    // turn. Yaw maps straight through, so x is the mirrored one; left
    // un-negated the camera slides right as the view turns left.
